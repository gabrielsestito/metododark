...
const toastVariants = cva(
  "group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-lg border p-6 pr-8 shadow-lg transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-top-full data-[state=open]:sm:slide-in-from-bottom-full",
  {
    variants: {
      variant: {
        default: "border bg-[#0f0f0f] text-white",
        success: "border-green-500/30 bg-gradient-to-r from-green-500/20 to-emerald-500/20 text-white",
        error: "border-red-500/30 bg-gradient-to-r from-red-500/20 to-rose-500/20 text-white",
        warning: "border-yellow-500/30 bg-gradient-to-r from-yellow-500/20 to-amber-500/20 text-white",
        info: "border-blue-500/30 bg-gradient-to-r from-blue-500/20 to-cyan-500/20 text-white",
      },
    },
    defaultVariants: {
      variant: "default",
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Action
    ref={ref}
    className={cn(
      "inline-flex h-8 shrink-0 items-center justify-center rounded-md border bg-transparent px-3 text-sm font-medium ring-offset-background transition-colors hover:bg-white/10 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 group-[.destructive]:border-red-500/50 group-[.destructive]:hover:border-red-500/70 group-[.destructive]:hover:bg-red-500/10",
      className
    )}
    {...props}
  />
